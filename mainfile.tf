...
resource "aws_security_group" "My-load_balancer_sg" {
...
resource "aws_security_group" "My-security-grp-rule" {
 ...
 ingress {
    ...
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
    security_groups = [aws_security_group.My-load_balancer_sg.id]
  }
 ingress {
    description = "HTTPS"
    from_port   = 443
...
resource "aws_instance" "Altschool1" {
  ami             = "ami-0b71701f667d37ddf"
  instance_type   = "t2.micro"
  key_name        = "KELE"
  security_groups = [aws_security_group.My-security-grp-rule.id]
  subnet_id       = aws_subnet.Altschool-public-subnet1.id
  availability_zone = "ca-central-1a"
  tags = {
    Name   = "Altschool-1"
...
# Creating file to store the IP addresses of the instances
resource "local_file" "Ip_address" {
  filename = "/root/miniproject/host-inventory"
  content  = <<EOT
